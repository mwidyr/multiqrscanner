...
public class InboundDetailsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    boolean isLoading;
    Activity activity;
    List<InventoryDetail> items;
    int visibleThreshold = 5;
    int lastVisibleItem, totalItemCount;

    public InboundDetailsAdapter(RecyclerView recyclerView, Activity activity, List<InventoryDetail> items) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof ItemViewHolder) {
            InventoryDetail item = items.get(position);
